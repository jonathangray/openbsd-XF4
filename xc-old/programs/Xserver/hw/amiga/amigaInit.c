...
)
{
	...
	amigaPtrPriv.fd = open ("/dev/mouse0", O_RDWR, 0);
...
#define WANT_SIGNALS(fd) fcntl(fd, F_SETOWN, getpid())
	...
	if (SET_FLOW(amigaKbdPriv.fd) == -1 || WANT_SIGNALS(amigaKbdPriv.fd) == -1) {	
	...
	if (SET_FLOW(amigaPtrPriv.fd) == -1 || WANT_SIGNALS(amigaPtrPriv.fd) == -1) {	
