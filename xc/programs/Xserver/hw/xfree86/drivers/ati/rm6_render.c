...
static void Rm6Init3DEngineMMIO(ScrnInfoPtr pScrn);
 ...
 */
static const struct blendinfo Rm6BlendOp[] = {
    /* Clear */
...
#define Rm6OpMax (sizeof(Rm6BlendOp) / sizeof(Rm6BlendOp[0]))
...
static CARD32 RM6TextureFormats[] = {
    PICT_a8r8g8b8,
    PICT_a8,
    PICT_x8r8g8b8,
    PICT_r5g6b5,
...
static CARD32
Rm6GetTextureFormat(CARD32 format)
{
    switch (format) {
    case PICT_a8r8g8b8:
...
static CARD32
Rm6GetColorFormat(CARD32 format)
{
    switch (format) {
    case PICT_a8r8g8b8:
    case PICT_x8r8g8b8:
 ...
 */
static CARD32
Rm6GetBlendCntl(CARD8 op, CARD32 dstFormat)
{
    CARD32 blend_cntl;

    if (op >= Rm6OpMax || Rm6BlendOp[op].blend_cntl == 0)
	return 0;

    blend_cntl = Rm6BlendOp[op].blend_cntl;
	
    if (Rm6BlendOp[op].dst_alpha && !PICT_FORMAT_A(dstFormat)) {
...
static void Rm6Init3DEngine(ScrnInfoPtr pScrn)
{
    RM6InfoPtr info = RM6PTR (pScrn);

	Rm6Init3DEngineMMIO(pScrn);
...
{
   RM6InfoPtr info = (RM6InfoPtr)(linear->devPrivate.ptr);
...
static Bool RM6SetupRenderByteswap(ScrnInfoPtr pScrn, int tex_bytepp)
{
    RM6InfoPtr info = RM6PTR(pScrn);
...
static void RM6RestoreByteswap(RM6InfoPtr info)
{
    unsigned char *RM6MMIO = info->MMIO;
...
static void FUNC_NAME(Rm6Init3DEngine)(ScrnInfoPtr pScrn)
{
    RM6InfoPtr  info       = RM6PTR(pScrn);
    ACCEL_PREAMBLE();
	...
	int flags)
{
    ...
    txformat = Rm6GetTextureFormat(format);
    tex_bytepp = PICT_FORMAT_BPP(format) >> 3;

#if X_BYTE_ORDER == X_BIG_ENDIAN
    if (!RM6SetupRenderByteswap(pScrn, tex_bytepp)) {
	xf86DrvMsg(pScrn->scrnIndex, X_ERROR, "%s: RM6SetupRenderByteswap() "
		   "failed!\n", __func__);
	return FALSE;
    }
#endif
...
) 
{
    ...
    blend_cntl = Rm6GetBlendCntl(op, dstFormat);
    if (blend_cntl == 0)
	return FALSE;

    if (!info->RenderInited3D)
	Rm6Init3DEngine(pScrn);

    if (!FUNC_NAME(R100SetupTexture)(pScrn, maskFormat, alphaPtr, alphaPitch,
				     width, height, flags))
	return FALSE;

    colorformat = Rm6GetColorFormat(dstFormat);
