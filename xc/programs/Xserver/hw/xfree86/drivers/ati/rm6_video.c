/* $XFree86: xc/programs/Xserver/hw/xfree86/drivers/ati/rm6_video.c,v 1.30 2003/11/10 18:22:18 tsi Exp $ */
...
static void RM6InitOffscreenImages(ScreenPtr);

static XF86VideoAdaptorPtr RM6SetupImageVideo(ScreenPtr);
static int  RM6SetPortAttribute(ScrnInfoPtr, Atom, INT32, pointer);
static int  RM6GetPortAttribute(ScrnInfoPtr, Atom ,INT32 *, pointer);
static void RM6StopVideo(ScrnInfoPtr, pointer, Bool);
static void RM6QueryBestSize(ScrnInfoPtr, Bool, short, short, short, short,
			unsigned int *, unsigned int *, pointer);
static int  RM6PutImage(ScrnInfoPtr, short, short, short, short, short,
			short, short, short, int, unsigned char*, short,
			short, Bool, RegionPtr, pointer);
static int  RM6QueryImageAttributes(ScrnInfoPtr, int, unsigned short *,
			unsigned short *,  int *, int *);

static void RM6VideoTimerCallback(ScrnInfoPtr pScrn, Time now);
...
} RM6PortPrivRec, *RM6PortPrivPtr;


#define GET_PORT_PRIVATE(pScrn) \
   (RM6PortPrivPtr)((RM6PTR(pScrn))->adaptor->pPortPrivates[0].ptr)


void RM6InitVideo(ScreenPtr pScreen)
{
    ...
    newAdaptor = RM6SetupImageVideo(pScreen);
    RM6InitOffscreenImages(pScreen);
...
static void
RM6SetOverlayGamma(ScrnInfoPtr pScrn, CARD32 gamma)
{
...
static void RM6SetTransform (ScrnInfoPtr pScrn,
				float	    bright,
				float	    cont,
				float	    sat,
				float	    hue,
				...
				CARD32      user_gamma)
{
    ...
    /* set gamma */
    RM6SetOverlayGamma(pScrn, gamma);
...
static void RM6SetColorKey(ScrnInfoPtr pScrn, CARD32 colorKey)
{
    RM6InfoPtr info = RM6PTR(pScrn);
    unsigned char *RM6MMIO = info->MMIO;
    CARD32 min, max;
    ...
    RM6WaitForFifo(pScrn, 2);
    OUTREG(RADEON_OV0_GRAPHICS_KEY_CLR_HIGH, max);
    OUTREG(RADEON_OV0_GRAPHICS_KEY_CLR_LOW, min);
}

void
RM6ResetVideo(ScrnInfoPtr pScrn)
{
    RM6InfoPtr   info      = RM6PTR(pScrn);
    unsigned char *RM6MMIO = info->MMIO;
    RM6PortPrivPtr pPriv = info->adaptor->pPortPrivates[0].ptr;
    ...
    */
    ...
    RM6SetColorKey(pScrn, pPriv->colorKey);
    ...
    RM6SetOverlayGamma(pScrn, 0); /* gamma = 1.0 */

}


static XF86VideoAdaptorPtr
RM6AllocAdaptor(ScrnInfoPtr pScrn)
{
    XF86VideoAdaptorPtr adapt;
    RM6InfoPtr info = RM6PTR(pScrn);
    RM6PortPrivPtr pPriv;
    unsigned char *RM6MMIO = info->MMIO;
    CARD32 dot_clock;
    ...
    if(!(pPriv = xcalloc(1, sizeof(RM6PortPrivRec) + sizeof(DevUnion))))
    {
	xfree(adapt);
	return NULL;
    }
...
static XF86VideoAdaptorPtr
RM6SetupImageVideo(ScreenPtr pScreen)
{
    ScrnInfoPtr pScrn = xf86Screens[pScreen->myNum];
    RM6PortPrivPtr pPriv;
    XF86VideoAdaptorPtr adapt;

    if(!(adapt = RM6AllocAdaptor(pScrn)))
	return NULL;
    ...
    adapt->GetVideo = NULL;
    adapt->GetStill = NULL;
    adapt->StopVideo = RM6StopVideo;
    adapt->SetPortAttribute = RM6SetPortAttribute;
    adapt->GetPortAttribute = RM6GetPortAttribute;
    adapt->QueryBestSize = RM6QueryBestSize;
    adapt->PutImage = RM6PutImage;
    adapt->QueryImageAttributes = RM6QueryImageAttributes;

    pPriv = (RM6PortPrivPtr)(adapt->pPortPrivates[0].ptr);
...
static void
RM6StopVideo(ScrnInfoPtr pScrn, pointer data, Bool cleanup)
{
  RM6InfoPtr info = RM6PTR(pScrn);
  unsigned char *RM6MMIO = info->MMIO;
  RM6PortPrivPtr pPriv = (RM6PortPrivPtr)data;
...
static int
RM6SetPortAttribute(ScrnInfoPtr  pScrn,
		       Atom	    attribute,
		       INT32	    value,
		       pointer	    data)
{
...
static int
RM6GetPortAttribute(ScrnInfoPtr  pScrn,
		       Atom	    attribute,
		       INT32	    *value,
		       pointer	    data)
{
...
RM6DisplayVideo(
    ...
    /* Unlike older Mach64 chips, RM6 has only two ECP settings: 0 for PIXCLK < 175Mhz, and 1 (divide by 2)
       for higher clocks, sure makes life nicer
    ...
    if (info->MergedFB) {
        if (info->OverlayOnCRTC2)
	    overlay_mode = ((RM6MergedDisplayModePtr)info->CurrentLayout.mode->Private)->CRT2;
	else
	    overlay_mode = ((RM6MergedDisplayModePtr)info->CurrentLayout.mode->Private)->CRT1;
    ...
    if (info->MergedFB) {
	RM6Scrn2Rel srel =
	    ((RM6MergedDisplayModePtr)info->CurrentLayout.mode->Private)->CRT2Position;
	overlay_mode = ((RM6MergedDisplayModePtr)info->CurrentLayout.mode->Private)->CRT2;
	if (srel == rm6LeftOf) {
...
RM6PutImage(
  ...
  RegionPtr clipBoxes, pointer data
   ...
   if (info->MergedFB)
	RM6ChooseOverlayCRTC(pScrn, &dstBox);
   ...
   if(!(info->videoLinear = RM6AllocateMemory(pScrn, info->videoLinear,
   ...
   switch(id) {
...
#endif
	RM6CopyMungedData(buf + (top * srcPitch) + left, buf + s2offset,
	...
	OUTREG(RADEON_SURFACE_CNTL, info->ModeReg.surface_cntl &
				    ~(RADEON_NONSURF_AP0_SWP_32BPP
				      | RADEON_NONSURF_AP0_SWP_16BPP));
#endif
	RM6CopyData(buf, dst_start, srcPitch, dstPitch, nlines, npixels);
	break;
    }
    ...
    RM6DisplayVideo(pScrn, id, offset, offset, width, height, dstPitch,
		     xa, xb, ya, &dstBox, src_w, src_h, drw_w, drw_h);

    pPriv->videoStatus = CLIENT_VIDEO_ON;

    info->VideoTimerCallback = RM6VideoTimerCallback;
...
static void
RM6VideoTimerCallback(ScrnInfoPtr pScrn, Time now)
{
    RM6InfoPtr info = RM6PTR(pScrn);
    RM6PortPrivPtr pPriv = info->adaptor->pPortPrivates[0].ptr;
...
RM6AllocateSurface(
    ...
    if(!(linear = RM6AllocateMemory(pScrn, NULL, size)))
	return BadAlloc;
...
RM6StopSurface(
  ...
  RM6InfoPtr info = RM6PTR(surface->pScrn);
  unsigned char *RM6MMIO = info->MMIO;
...
static int
RM6GetSurfaceAttribute(
    ScrnInfoPtr pScrn,
    Atom attribute,
    INT32 *value
){
   return RM6GetPortAttribute(pScrn, attribute, value,
		(pointer)(GET_PORT_PRIVATE(pScrn)));
}

static int
RM6SetSurfaceAttribute(
    ScrnInfoPtr pScrn,
    Atom attribute,
    INT32 value
){
   return RM6SetPortAttribute(pScrn, attribute, value,
		(pointer)(GET_PORT_PRIVATE(pScrn)));
}
...
RM6DisplaySurface(
    ...
    RM6InfoPtr info = RM6PTR(pScrn);
    RM6PortPrivPtr portPriv = info->adaptor->pPortPrivates[0].ptr;
    ...
    RM6ResetVideo(pScrn);
#endif
    RM6DisplayVideo(pScrn, surface->id,
...
static void
RM6InitOffscreenImages(ScreenPtr pScreen)
{
/*  ScrnInfoPtr pScrn = xf86Screens[pScreen->myNum];
    RM6InfoPtr info = RM6PTR(pScrn); */
    ...
    offscreenImages[0].alloc_surface = RM6AllocateSurface;
    offscreenImages[0].free_surface = RM6FreeSurface;
    offscreenImages[0].display = RM6DisplaySurface;
    offscreenImages[0].stop = RM6StopSurface;
    offscreenImages[0].setAttribute = RM6SetSurfaceAttribute;
    offscreenImages[0].getAttribute = RM6GetSurfaceAttribute;
